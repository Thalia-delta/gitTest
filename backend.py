...
def analyze_with_multimodal_model(photo_data):
    ...
    try:
        ...
        messages = [
            {
                "role": "system",
                "content": "你是专业的中文旅行轨迹梳理专家，专注于通过分析照片本身内容、以及照片元数据（EXIF时间、GPS、用户标签），系统化梳理用户的旅行记忆，生成结构化、易读的旅行轨迹报告。你的目标是精准还原旅行动线并提供多风格的记忆输出方案。\n"
                           "1. 为整个相册生成一个合适的中文名称和一句中文描述\n"
                           "2. 必须根据EXIF信息中的地点、时间顺序排列形成旅行路径、不许根据图中内容杜撰地点\n"
                           "3. 为每个地点生成三种中文描述：\n"
                           "   - 极简回忆：简洁的地点+活动描述\n"
                           "   - 故事叙述：详细的旅行故事\n"
                           "   - 社交分享：适合发社交媒体的风格，包含适当的emoji\n"
                           "4. 为每个地点选择一张代表性照片（使用照片索引）\n"
                           "5. 为每个地点提供EXIF信息中的经纬度坐标\n"
